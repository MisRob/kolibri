...
} = AssessmentWrapper.$trs;

const ASSESSMENT_IDS = ['item-1', 'item-2', 'item-3', 'item-4', 'item-5'];
const DEFAULT_MASTERY_MODEL = { type: 'm_of_n', m: 3, n: 5 };

function goalText(count) {
  return goal.message
    .replace('{count, number, integer}', count)
    .replace('{count, plural, other {correct}}', 'correct');
}
...
describe('AssessmentWrapper', () => {
  ...
  describe('rendering', () => {
    it('renders the mastery goal text', () => {
      ...
      const elements = screen.getAllByText(goalText(3));
  ...
  describe('checkAnswer with no answer', () => {
    it('shows "Please enter an answer above" when check is clicked but content viewer returns null', async () => {
      renderComponent();
      await fireEvent.click(screen.getByText(check.message));
      expect(getStatusText()).toBe(inputAnswer.message);
    });
  ...
  describe('correct answer on first attempt', () => {
    ...
    it('shows the Next button after a correct answer', async () => {
      ...
      expect(screen.queryByText(check.message)).not.toBeInTheDocument();
    });
    ...
    it('shows "Correct!" status when pastattempts contain a correct attempt', async () => {
      ...
      await fireEvent.click(screen.getByText(check.message));
      expect(getStatusText()).toBe(correct.message);
    });
  ...
  describe('incorrect answer', () => {
    ...
    it('shows "Try again" after an incorrect answer', async () => {
      ...
      await fireEvent.click(screen.getByText(check.message));
      expect(getStatusText()).toBe(tryAgain.message);
    });
    ...
    it('adds the shaking class on the Check button', async () => {
      renderWithIncorrectViewer();
      await fireEvent.click(screen.getByText(check.message));
      const checkButton = screen.getByText(check.message).closest('button');
  ...
  describe('rectified answer', () => {
    it('shows "Great! Keep going" when answer is corrected on second attempt', async () => {
      ...
      await fireEvent.click(screen.getByText(check.message));
      expect(getStatusText()).toBe(greatKeepGoing.message);
    });
    ...
    it('does not send progress on the second attempt', async () => {
      ...
      await fireEvent.click(screen.getByText(check.message)); // first attempt
      await fireEvent.click(screen.getByText(check.message)); // second attempt
  ...
  describe('nextQuestion', () => {
    it('resets state and shows Check button after clicking Next', async () => {
      ...
      await fireEvent.click(screen.getByText(next.message));
      expect(screen.getByText(check.message)).toBeInTheDocument();
      expect(screen.queryByText(next.message)).not.toBeInTheDocument();
    });
  });
  ...
  describe('item error handling', () => {
    it('shows error alert with "Try a different question" button', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(itemError.message)).toBeInTheDocument();
        expect(screen.getByText(tryDifferentQuestion.message)).toBeInTheDocument();
      });
    });
    ...
    it('clicking "Try a different question" resets to a new question', async () => {
      ...
      await fireEvent.click(screen.getByText(tryDifferentQuestion.message));

      await waitFor(() => {
        expect(screen.queryByText(itemError.message)).not.toBeInTheDocument();
        expect(screen.getByText(check.message)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('mastery model variations in UI', () => {
    it('shows correct goal for do_all model', () => {
      ...
      expect(screen.getAllByText(goalText(2)).length).toBeGreaterThanOrEqual(1);
    });
    ...
    it('shows correct goal for num_correct_in_a_row_3', () => {
      ...
      expect(screen.getAllByText(goalText(3)).length).toBeGreaterThanOrEqual(1);
    });

    it('shows correct goal for num_correct_in_a_row_10', () => {
      renderComponent({ masteryModel: { type: 'num_correct_in_a_row_10' } });
      expect(screen.getAllByText(goalText(10)).length).toBeGreaterThanOrEqual(1);
    });
